#!/usr/bin/env python3
"""
Sweep many MLP architectures on 3-qubit datasets:
    dataset3_F1.csv  (10 features)
    dataset3_F2.csv  (27 features)

- Datasets are assumed to live in your zhome:
    /zhome/b8/7/168711/dataset3_F1.csv
    /zhome/b8/7/168711/dataset3_F2.csv

- For each dataset ("F1" or "F2") and architecture:
...
Example usage on HPC:
    python quibt3F1F2NNSWEEP.py --datasets F1 F2 --epochs 100 --batch_size 1024 --lr 1e-3 --patience 15
"""
...
# 3-qubit datasets
DATA_PATH_F1 = os.path.join(ZHOME_BASE, "dataset3_F1.csv")
DATA_PATH_F2 = os.path.join(ZHOME_BASE, "dataset3_F2.csv")

RESULTS_BASE_PATH = os.path.join(ZHOME_BASE, "quantum_3q_nn_results")
...
class MLP(nn.Module):
    def __init__(self, input_dim: int, hidden_sizes, dropout=0.0):
        ...
        # 1D regression output (GlobalEntanglement)
...
def load_dataset(dataset_name: str, logger: Logger):
    """
    Load 3-qubit F1 or F2 dataset from CSV and return (X, y, feature_cols).
    Label column is assumed to be 'GlobalEntanglement'.
    """
    if dataset_name == "F1":
        path = DATA_PATH_F1
        feature_cols = [
            "F1_xxx", "F1_xxy", "F1_xxz",
            "F1_xyy", "F1_xyz", "F1_xzz",
            "F1_yyy", "F1_yyz", "F1_yzz", "F1_zzz",
        ]
    elif dataset_name == "F2":
        path = DATA_PATH_F2
        feature_cols = [
            "F2_xxx", "F2_xxy", "F2_xxz",
            "F2_xyx", "F2_xyy", "F2_xyz",
            "F2_xzx", "F2_xzy", "F2_xzz",
            "F2_yxx", "F2_yxy", "F2_yxz",
            "F2_yyx", "F2_yyy", "F2_yyz",
            "F2_yzx", "F2_yzy", "F2_yzz",
            "F2_zxx", "F2_zxy", "F2_zxz",
            "F2_zyx", "F2_zyy", "F2_zyz",
            "F2_zzx", "F2_zzy", "F2_zzz",
        ]
    else:
    ...
    logger.log(f"Loading {dataset_name} (3-qubit) from: {path}")
    df = pd.read_csv(path)

    # Target column
    label_col = "GlobalEntanglement"
    if label_col not in df.columns:
        raise KeyError(f"'{label_col}' column not found in {path}.")
...
def make_dataloaders(X, y, batch_size: int, logger: Logger):
    ...
    # Standardize features using train statistics (labels are left as-is)
...
def train_one_architecture(
    ...
    run_name = f"3q_{dataset_name}_{arch_name}_{timestamp}"
    ...
    # ---- Load data ----
    X, y, feature_cols, label_col = load_dataset(dataset_name, logger)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_mse < best_val_mse - 1e-8:
            ...
            logger.log(
                ...
                f"with Val MSE {best_val_mse:.6f}"
            )
        else:
            epochs_no_improve += 1
...
def main():
    parser = argparse.ArgumentParser(
        description="Sweep MLP architectures on 3-qubit F1/F2 datasets"
    )
    parser.add_argument(
        "--datasets",
        nargs="+",
    ...
    summary_csv_path = os.path.join(
        RESULTS_BASE_PATH, f"3q_F_sweep_results_{timestamp}.csv"
    )
